"""
Vocab Sentence Generator — Anki Add-on
Reads the Word field, generates example sentences via Claude AI,
and writes them all into the Front field. The add-on uses the
card_will_show hook to pick one sentence per review and inject it
directly — so it never changes mid-session.
"""
...
# Maps card_id -> chosen sentence HTML for the current review session.
# Cleared when a new review session starts (main window re-init).
...
def on_card_will_show(html: str, card, kind: str) -> str:
    ...
    try:
        ...
        if "Front TTS" in note:
            plain = re.sub(r"<[^>]+>", "", chosen).strip()
            if note["Front TTS"] != plain:
                note["Front TTS"] = plain
                note.flush()
    except Exception:
        pass
...
def run_generation(editor):
    ...
    # Clear any cached session pick for this note so it gets a fresh pick
    if editor.note and editor.note.id:
        _session_picks.pop(editor.note.id, None)
...
class SettingsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Vocab Sentence Generator — Settings")
...
def setup_menu():
    submenu = QMenu("Vocab Sentence Generator", mw)
